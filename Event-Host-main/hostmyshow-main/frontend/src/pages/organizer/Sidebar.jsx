...
import { LayoutDashboard, CalendarPlus, List, Ticket, Star, Mail, BarChart3 } from 'lucide-react';
  ...
  <aside className="w-64 glass p-6 shadow-lg flex flex-col">
    ...
    <nav className="flex-1">
      <ul className="space-y-4">
        ...
        <li>
          <NavLink
            to="/organizer/host-analysis"
            className={({ isActive }) =>
              `flex items-center gap-3 text-lg transition-colors px-3 py-2 rounded-lg ${isActive ? 'bg-blue-700 text-white' : 'hover:text-blue-300'}`
            }
          >
            <BarChart3 className="w-5 h-5" /> Host Analysis
          </NavLink>
        </li>
      </ul>
    </nav>
