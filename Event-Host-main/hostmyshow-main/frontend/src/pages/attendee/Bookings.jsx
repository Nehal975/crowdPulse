  ...
  return (
    <div className="glass rounded-xl shadow-lg overflow-hidden flex flex-col items-center relative">
      ...
      <div className="p-6 flex flex-col flex-1 w-full items-center">
        ...
        <div className="text-sm text-blue-100 mb-1">Seat: {booking.seats}</div>
...
const MyBookings = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      // Only keep one booking per event (latest booking)
      const uniqueBookings = [];
      const eventIds = new Set();
      for (let i = res.data.bookings.length - 1; i >= 0; i--) {
        const booking = res.data.bookings[i];
        const eventId = booking.event_id?._id || booking.event_id;
        if (!eventIds.has(eventId)) {
          uniqueBookings.unshift(booking);
